...
# Reachy Mini conversation app (MiniMax fork)

Conversational app for the Reachy Mini robot. This fork replaces the OpenAI realtime pipeline with a **MiniMax + ElevenLabs + Whisper** stack while keeping the original vision, motion, and tool infrastructure intact.
...
## Overview
- Real-time audio conversation loop using **Silero VAD** (fastrtc `ReplyOnPause`) → **faster-whisper** STT → **MiniMax-M2.7** LLM → **ElevenLabs** TTS.
- TTS audio is played on **Reachy Mini's physical speaker** (GStreamer pipeline) in addition to the browser/Gradio stream.
- Vision processing uses the MiniMax chat completions API by default (when camera tool is used), with optional on-device local vision using SmolVLM2 (CPU/GPU/MPS) via `--local-vision`.
...
| Stage | Original | This fork |
|-------|----------|-----------|
| VAD | OpenAI Realtime (server-side) | Silero VAD via `fastrtc` `ReplyOnPause` |
| STT | OpenAI Realtime | `faster-whisper` (local, Whisper `base` model, CPU int8) |
| LLM | OpenAI GPT-4o Realtime | MiniMax-M2.7 chat completions at `https://api.minimax.io/v1` |
| TTS | OpenAI Realtime | ElevenLabs (`eleven_turbo_v2_5`, PCM 24 kHz) |
| Speaker | Browser/Gradio only | **Reachy Mini physical speaker** + browser/Gradio |

### Key implementation details

- **`src/reachy_mini_conversation_app/minimax_handler.py`** — New handler class `MinimaxHandler` (replaces `OpenaiRealtimeHandler`):
  - Lazy-initialises `AsyncOpenAI` client pointed at MiniMax and `AsyncElevenLabs` client.
  - Concurrent-turn lock (`asyncio.Lock`) prevents a second speech turn from corrupting chat history while a tool call is in flight.
  - `_transcribe()` runs faster-whisper in a thread pool executor (non-blocking).
  - `_tts_stream()` streams ElevenLabs PCM audio and simultaneously pushes each chunk to Reachy Mini's GStreamer speaker pipeline (int16/24 kHz/mono → float32/16 kHz/stereo via `resample_poly`).

- **`src/reachy_mini_conversation_app/config.py`** — Added `ELEVENLABS_API_KEY`, `ELEVENLABS_VOICE_MAP` (12 ElevenLabs voices), and `DEFAULT_VOICE = "Rachel"`.

- **`src/reachy_mini_conversation_app/gradio_personality.py`** — Added `allow_custom_value=True` to the voice `Dropdown` and a `blocks.load` event that initialises the tools `CheckboxGroup` with real choices on page load (prevents stale browser cache from causing validation errors).

- **`src/reachy_mini_conversation_app/prompts.py`** — Default voice changed from `Cute_Girl` to `Rachel`.

- **`profiles/example/voice.txt`** — Created; contains `Rachel`.

### Additional dependencies

```bash
uv add "elevenlabs>=1.0.0" "fastrtc[vad]" "faster-whisper"
```
...
|----------|-------------|
| `OPENAI_API_KEY` | Required. Your MiniMax API key (used for the chat completions endpoint). |
| `OPENAI_BASE_URL` | Set to `https://api.minimax.io/v1` for MiniMax. |
| `MODEL_NAME` | MiniMax model to use (defaults to `MiniMax-M2.7`). |
| `ELEVENLABS_API_KEY` | Required. Your ElevenLabs API key for TTS voice synthesis. |
...
| `REACHY_MINI_CUSTOM_PROFILE` | Profile name to load from `profiles/`. Defaults to `default` when unset. |

**Minimal `.env` example for MiniMax + ElevenLabs:**
